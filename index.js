...
Connection.prototype._queryHandler = async function (data, callback) {
  ...
  function buildQuery(query, parameters, defaults) {
    return query.replace(/:([a-z0-9]+)/gi, (match, parameterName) => {
      if (!parameters || !parameterName) {
        return '';
      }

      const newValue = parameters[parameterName];

      function convertValue(replacedValue) {
      ...
      if (Array.isArray(newValue)) {
        if (defaults && newValue.length === 0) {
          const defaultValue = defaults[parameterName];
          if (defaultValue === undefined || defaultValue === null) {
            return '';
          }
          return defaultValue;
        }
        return newValue.map(convertValue).join(',');
      } else {
        return convertValue(newValue);
      }
    }).trim();
  }
  ...
  if (match) {
    if (!opts.fields || opts.fields === 'auto') {
      ...
      opts.query = buildQuery(
        opts.query.replace('*', opts.fields
          .map((field) => isField(field) ? field.name : field)
          .join(', ')),
        opts.parameters,
        opts.defaults,
      );
    } else {
      opts.query = buildQuery(opts.query, opts.parameters, opts.defaults);
    }
  }
  opts.qs = {
    q: opts.query
